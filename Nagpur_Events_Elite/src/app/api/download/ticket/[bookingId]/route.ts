...
export async function GET(req: NextRequest, { params }: { params: { bookingId: string } }) {
  try {
    ...
    const fileUrl = booking.ticketPdfUrl || booking.ticketPdfPath;
    if (!fileUrl) return NextResponse.json({ error: 'Ticket file not available' }, { status: 404 });
    ...
    headers.set('Content-Disposition', `attachment; filename="Ticket-${booking.ticketId}.pdf"`);
